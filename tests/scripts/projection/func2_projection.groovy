...
[
    ...
    "SELECT ac.id, (ac.income - ac.tax), (ac.income - 2), (2 - ac.income), (1 - 2) FROM `Actor` ac",
    ...
    "SELECT ac.id, (ac.income * ac.incomeFactor), (ac.income * 2), (2 * ac.income), (2 * 3) FROM `Actor` ac",
    ...
    "SELECT ac.id, (ac.income / ac.familyCount), (ac.income / 2), (2 / ac.income), (22 / 7) FROM `Actor` ac",
    ...
    "SELECT ac.id, (ac.income % ac.spent), (ac.income % 2), (2 % ac.income), (22 % 7) FROM `Actor` ac",

    $DSL.select {
        TARGET (Actor.alias("ac"))
        FETCH (ac.id, INVERSE(ac.income))
    },
    "SELECT ac.id, (1 / ac.income) FROM `Actor` ac",

    $DSL.select {
        TARGET (Actor.alias("ac"))
        FETCH (ac.id, LCASE(ac.name), UCASE(ac.lastName), TRIM(ac.details), LEN(ac.countryCode))
    },
    "SELECT ac.id, LOWER(ac.name), UPPER(ac.lastName), TRIM(ac.details), CHAR_LENGTH(ac.countryCode) FROM `Actor` ac",

    $DSL.select {
        TARGET (Actor.alias("ac"))
        FETCH (REVERSE(ac.nameHash), LEFT_TRIM(ac.city), RIGHT_TRIM(ac.country))
    },
    "SELECT REVERSE(ac.nameHash), LTRIM(ac.city), RTRIM(ac.country) FROM `Actor` ac",
    ...
    ["SELECT LEFT(ac.details, 320), LEFT(ac.details, ?), " +
    ...
    ["SELECT LPAD(ac.nickName, 10, \" \"), LPAD(ac.nickName, 10, \"-\"), " +
    ...
    ["SELECT POWER(ac.rank, 3), POWER(ac.rank, ?), POWER(2, 3), " +
    ...
    ["SELECT CAST(ac.birthDay AS SIGNED), CAST(ac.level AS CHAR), CAST(ac.retiredDate AS DATE), " +
